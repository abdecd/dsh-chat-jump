	...
	factory: (require) => {
		...
		function ChatHistoryQuickJump({ messages = [], retryableTurns = [], disableNativeTurnNav: disableNativeTurnNavProp }) {
			...
			(0, react.useEffect)(() => {
				...
				const navElements = document.querySelectorAll("nav[aria-label=\"轮次导航\"], nav[aria-label=\"Turn navigation\"], nav[aria-label*=\"导航\"], nav[aria-label*=\"navigation\" i]");
				for (const nav of navElements) {
					nav.dataset.dshTurnNavDisabled = "true";
					if (nav.parentElement && nav.parentElement !== document.body) nav.parentElement.dataset.dshTurnNavDisabled = "true";
				}
				const chatFlow = document.querySelector("[data-chat-flow]");
